...
import { technicalSpecialist } from '../../specialists/technical/index.js';
import type { ArchitectureNote } from '@lucid/schema';
import type { RequirementItem } from '@lucid/schema';

/** Stage 6: run the TECHNICAL specialist and merge its decisions into state. */
export function technicalSpecialistPass(state: PipelineState): PipelineState {
  const decisions = technicalSpecialist(state);
  ...
  return {
    ...
    stagesRun: [...state.stagesRun, 'technicalSpecialistPass'],
  };
}
