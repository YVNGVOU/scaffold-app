...
import { qaSpecialist } from '../../specialists/qa/index.js';
import type { ArchitectureNote } from '@lucid/schema';
import type { RequirementItem } from '@lucid/schema';

/** Stage: run the QA specialist and merge its decisions into state. */
export function qaSpecialistPass(state: PipelineState): PipelineState {
  const decisions = qaSpecialist(state);
  ...
  return {
    ...
    stagesRun: [...state.stagesRun, 'qaSpecialistPass'],
  };
}
