...
import { securitySpecialist } from '../../specialists/security/index.js';
import type { ArchitectureNote } from '@lucid/schema';
import type { RequirementItem } from '@lucid/schema';

/** Stage 8: run the SECURITY specialist and merge its decisions into state. */
export function securitySpecialistPass(state: PipelineState): PipelineState {
  const decisions = securitySpecialist(state);
  ...
  return {
    ...
    stagesRun: [...state.stagesRun, 'securitySpecialistPass'],
  };
}
