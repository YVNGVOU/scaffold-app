  ...
  ambiguityChecklist: [
    {
      ...
      isResolved: (input) => /(print|digital|screen|web|social media|pdf|paper|physical)/i.test(input),
    },
    {
      field: 'dimensions',
      description: 'Final size/dimensions or aspect ratio of the piece is unspecified',
      isResolved: (input) => /(size|dimensions?|inches|cm|mm|a4|a3|letter size|\d+\s*x\s*\d+|aspect ratio)/i.test(input),
    },
    {
      field: 'color mode',
      description: 'Target color mode (CMYK for print vs. RGB for screen) is unspecified',
      isResolved: (input) => /(cmyk|rgb|color mode|pantone|spot color|full color|grayscale)/i.test(input),
    },
    {
      field: 'file format',
      description: 'Required deliverable file format(s) (e.g. PDF, AI, PNG, EPS) are unspecified',
      isResolved: (input) => /(pdf|\bai\b|eps|png|svg|tiff|jpe?g|editable file|source file)/i.test(input),
    },
  ],
